-- =============================================================
-- Waitlist v9: raise per-IP limit for viral/shared-network traffic
-- -------------------------------------------------------------
-- Launch readiness: the per-IP limit of 15 sign-ups/hour was tuned
-- to stop one machine flooding with many emails, but it is TOO TIGHT
-- for real viral traffic. In Egypt many mobile users share a single
-- carrier public IP (CGNAT), so 1000+ genuine sign-ups from one
-- shared IP would wrongly hit `ip_limited`.
--
-- Raised to 60 sign-ups/IP/hour - tolerates realistic shared-NAT
-- bursts while still stopping obvious scripted floods. The real
-- abuse backstops stay intact:
--   * unique index on lower(email)  -> no duplicates, no voucher reuse
--   * per-email limit 5/10min       -> no hammering one email
--   * honeypot + input validation   -> blocks dumb bots
--   * atomic row-locked count       -> exact referral counts
-- =============================================================
...
begin
  ...
  -- 3) Per-IP rate limit: max 60 sign-ups / hour (tolerates shared-NAT
  --    viral bursts, still stops obvious scripted floods).
  if v_ip <> '' then
